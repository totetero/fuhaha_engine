...
void engineLayoutOld01ButtonCalc(struct engineLayoutOld01Button *this){
	...
	engineLayoutOld01ButtonSubCalc(this, t, true);
	if(t != NULL){
...
void engineLayoutOld01ButtonSubCalc(struct engineLayoutOld01Button *this, struct engineCtrlTouch *t, bool clickable){
	if(this->inactive || !clickable){
		...
		// ボタン押下中
		this->active = (engineLayoutOld01ButtonIsInner(this, t) == !!this->inner);
		// 子要素の範囲内では押下状態にならない
		if(this->child != NULL){this->active = this->active && !(engineLayoutOld01ButtonIsInner(this->child, t) == !!this->child->inner);}
	}else if(this->active){
		// ボタンを放した瞬間
		this->active = false;
		this->trigger = true;
