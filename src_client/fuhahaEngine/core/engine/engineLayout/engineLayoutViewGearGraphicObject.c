...
void engineLayoutViewGearGraphicObjectDispose(struct engineLayoutView *this){
	if(this->graphicObject.next == this){
		...
		// 双方向循環リストから外す
		engineLayoutViewGearGraphicObjectDisconnect(this);
	}
...
void engineLayoutViewGearGraphicObjectBufferCreateAll(struct engineLayoutView *this){
	// バッファを作成する
	engineLayoutViewGearGraphicObjectBufferCreate(this);
	// 子要素に対して処理を適用する
	struct engineLayoutView *temp = this->family.childrenHead;
	while(temp != NULL){
		engineLayoutViewGearGraphicObjectBufferCreateAll(temp);
...
void engineLayoutViewGearGraphicObjectBufferActiveAll(struct engineLayoutView *this){
	...
	struct engineLayoutView *temp = this->family.childrenHead;
	while(temp != NULL){
		engineLayoutViewGearGraphicObjectBufferActiveAll(temp);
		temp = temp->family.next;
	}
}
