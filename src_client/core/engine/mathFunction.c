...
double mathSin(double x){
	x -= (int32_t)(x / (2 * CONST_PI)) * (2 * CONST_PI);
	// テイラー展開を行う
	double sum = x;
	double t = x;
...
double mathCos(double x){return mathSin(CONST_PI * 0.5 - x);}
...
static double mathAtan1(double x){
	...
	if(x >  1){return  CONST_PI * 0.5 - mathAtan1(1 / x);}
	if(x < -1){return -CONST_PI * 0.5 - mathAtan1(1 / x);}
	double a = 0;
...
double mathAtan2(double y, double x){
	...
	if(x < 0){return mathAtan1(y / x) + CONST_PI;}
