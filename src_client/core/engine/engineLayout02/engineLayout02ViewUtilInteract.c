...
bool engineLayout02ViewUtilInteractTouch(struct engineLayout02View *this, int touchIndex, double x, double y, bool dn, bool mv, bool isCancel){
	...
	if(!this->interact.status.isActive){
		...
		if(isCancel){
			...
			this->interact.status.currX = x;
			this->interact.status.currY = y;
			if(engineLayout02ViewUtilPositionTransformIsInner(this, x, y)){this->interact.status.isTriggerUp = true;}
			return true;
		}
	}else{
		// 関係がない
...
void engineLayout02ViewUtilInteractTouchRoot(struct engineLayout02View *this, bool isCancel){
	...
	for(int i = 0; i < (int)(sizeof(touchIdList) / sizeof(*touchIdList)); i++){
		...
		engineLayout02ViewTouch(this, i, t->screen.x, t->screen.y, t->dn, t->mv, isCancel);
	}
}
