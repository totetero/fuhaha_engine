...
// タッチ可能な子要素の外側にあることを再帰で確認する
static bool isChildrenOuter(struct engineLayoutView *this, double x, double y){
	// 子要素に対して処理を適用する
	struct engineLayoutView *temp = this->family.childrenHead;
	while(temp != NULL){
		if(temp->interact.setting.isTouchable && temp->position.isInner(temp, x, y)){return false;}
		if(!isChildrenOuter(temp, x, y)){return false;}
...
static bool isInner(struct engineLayoutViewPartsButtonOuterImplement *this, double x, double y){
	return engineLayoutViewUtilPositionTransformIsInner((struct engineLayoutView*)this, x, y) && isChildrenOuter((struct engineLayoutView*)this, x, y);
}
