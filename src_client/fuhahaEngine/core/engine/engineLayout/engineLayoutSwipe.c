...
bool engineLayoutSwipeTouch(struct engineLayoutSwipe *this, struct engineLayoutView *view, int touchIndex, double x, double y, bool dn, bool mv, bool isCancel){
	...
	bool isActiveChild = engineLayoutViewUtilChildrenTouch((struct engineLayoutView*)view, touchIndex, x, y, dn, mv, isCancel || isActive || isMove); isActive = isActiveChild || isActive;
	bool isChildrenMove = this->isChildrenMove;
	bool isActiveLocal = engineLayoutViewUtilInteractTouch((struct engineLayoutView*)view, touchIndex, x, y, dn, mv, isCancel || isChildrenMove); isActive = isActiveLocal || isActive;
	...
	if(isActiveLocal){
		...
		engineLayoutViewUtilPositionTransformCalcInvert((struct engineLayoutView*)view, &tempVec1);
		// 新タッチ位置更新
		this->x = tempVec1.x;
		this->y = tempVec1.y;
	}
